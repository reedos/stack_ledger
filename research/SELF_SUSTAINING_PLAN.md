...
- Datasets: Epoch AI data centers, GPU clusters, chip components, chip sales and companies imported as vintage-stamped snapshots with per-site and per-designer estimate series; 876 Epoch-cited primary URLs queued as leads. QCEW and QWI importers already existed (`scripts/import_qcew.py`, BLS county employment; `scripts/import_qwi.py`, Census hires and earnings) but ran only by hand; a reviewed `research/importers.json` now gives Epoch, QCEW and QWI each a cadence, weekday and timeout, and `scripts/nightly.py`'s `importers` stage runs whichever are due, commits their tracked changes under `research/`, `site/` and `docs/` and rolls the working tree back cleanly on any failure. Same-source refresh semantics already exist in `import_epoch.py` — a new vintage supersedes a promoted series wholesale, and a per-site dated snapshot only ever adds a point, never rewrites an earlier one (`import_epoch.py:275,416-418`) — and QCEW/QWI append new quarters the same way; B2 needed a schedule, not new refresh code.
...
B1. Importers, each following the Epoch pattern (download, hash, vintage, snapshot, promote, leads): SEC XBRL companyfacts for annual and quarterly revenue and capex of every directory company with a CIK; EIA electricity generation, capacity additions and the 860M generator inventory; BLS occupational employment for electricians and construction; Census construction spending; FERC and ISO interconnection queues where a CSV is published (ERCOT large-load list, PJM queue export).
B2. Refresh semantics for series: a new vintage of the same source and metric supersedes the previous snapshot automatically, keeping history, because nothing a person chose is replaced. Add this rule to `publication-policy.json` as "same-source refresh" and enforce it in the importer, so quarterly revenue and capex update themselves.
