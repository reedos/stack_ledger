...
- Datasets: Epoch AI data centers, GPU clusters, chip components, chip sales, companies and (added September 9, 2026) Notable AI Models imported as vintage-stamped snapshots with per-site, per-designer and yearly models-layer estimate series; the Epoch Capabilities Index snapshot behind `/explorers/` now refreshes on the same vintage discipline instead of a single hand-run pull; 876+ Epoch-cited primary URLs queued as leads. Census BTOS AI-use share (national plus NAICS 51/54) is a second Census-family importer alongside QCEW/QWI. `scripts/importer_common.py` is the shared apply lane (register/promote/validate/save/build, and, added September 10, 2026, land graded events) the direct-apply importers call, each landing as a recorded `importer_apply` receipt in the same event log the panel reads. Grid-interface tracking (September 10, 2026) added a fourth EIA series (retail electricity price by state), a Federal Register API importer publishing grade-A government-action events (`import_policy.py`, keyless, applied for real this pass), and a grid-operator source registry (`import_grid.py`) that names PJM/ERCOT/MISO/SPP/EIA and reports every one `needs_reviewed_import` rather than hand-entering a PDF number.
...
7. The model does not join `publication-policy.json`'s `auto_apply.authors` allowlist until a real precision/recall calibration gate clears an owner-set floor (D5, expanded to score every labeled case against its ground truth, not today's 2-of-8 tool). Two maintainer tools are listed as of 2026-09-10, `"Epoch import (maintainer tool)"` and `"Policy import (maintainer tool)"` (added for a future Federal Register import that chooses this lane; today's `import_policy.py` still writes directly through `apply_changes()`, never through this one, since this lane commits and pushes); QCEW, QWI, SEC, EIA and the direct-lane policy/grid importers do not need the allowlist because they write directly, following Epoch's own maintainer-tool pattern, not through the model.
