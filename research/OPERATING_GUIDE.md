...
`scripts/evaluate_extraction.py` measures whether the local model can do extraction and screening at all, separately from whether the nightly runner found anything to accept. It replays `.local/evidence` documents already on disk through the same `extract_note`/`extract_observations` code paths and the same reviewed prompts and validators, offline: no fetch, no publication, no ledger write. It refuses to run while `.local/research.lock` or `.local/research-session.lock` exists. Each run writes `results.json`, a `report.md` with one row per candidate for a human grader, and a `grades.template.json`; `--grade` turns a filled-in grades file into `scored.md` with accepted-record precision and the validator's and reviewer's false-reject/false-accept rates. A low yield in the nightly runner can mean the model is failing extraction, the screen is correctly rejecting weak candidates, or the corpus has little left to find; this tool is how to tell those apart before changing a prompt or a validator to chase yield.
