...
`research.py` supplies `CONSTITUTION.md` and `OPERATING_GUIDE.md` to monitoring extraction, skeptical screening and discovery. Private catalog drafting receives the same supplied instructions. Since September 9, every extraction task also receives `EVIDENCE_RULES` and every reviewer pass receives `SCREENING_RULES` from `scripts/model_rules.py`. Those two short blocks restate exactly what the deterministic validators enforce (contiguous copied evidence, every stated number inside the excerpt except the source publication year, no angle brackets, glued unit letters count as the number) and hold the monitoring reviewers and the discovery screen to one standard. A quote that differs from the source only in typography or whitespace is snapped to the document's own bytes before hashing; a quote stitched from two passages is still rejected. Reviewers see the located passage with surrounding text, not the whole document.

`runtime.json` `instructions` selects what the model receives: `full` (constitution plus operating guide, about 11k tokens) or `brief` ([MODEL_BRIEF.md](MODEL_BRIEF.md), about 2k tokens, the decision routine, vocabulary, distinctions, evidence and screening rules and worked examples). `research.py --instructions brief` overrides for one run. The brief is reviewed instruction; switching modes changes the cache identity, so the first run after a switch re-screens cached documents. Reviewer passes now fill a checklist (numbers_in_evidence, scope_matches, basis_correct, attribution_correct) and name a defect code; the runner derives supported and prefixes quarantine reasons with the code, which are the reason codes below. Every due source is read, and every one now gets a note-lane attempt: any source without excerpt permission and no parent (discovered) page -- whether or not it has a linked metric -- produces a private note in `.local/review-candidates/` marked `publication: private`, never a public event, capped per run by `max_private_notes_per_run`. Since September 11 a second cap, `max_private_note_calls_per_session` (runtime.json, default 12), bounds the model calls spent on documents whose note could only ever be private, counted across the session rather than per batch; a source that can publish never consults it. `python scripts/registry_lint.py` lists them with the sources that fail most often. The operating guide contains the compact decision routine and worked examples; this longer playbook is a maintainer reference, not a file the model can independently open. The saved-evidence discovery replay loads the same operating guide.
...
**Calibration status: started, not the full 20-case suite.** `scripts/evaluate_extraction.py --holdout` (see the Operating Guide's "Measuring the model" and its September 9 "Validator recall pass") is the extraction/screening half of this calibration procedure: a bounded, offline, saved-evidence replay against the real corpus with publication disabled, exactly what step 4 above asks for, though it draws its 24-document sample by seed rather than the hand-picked 20-case purposive/stratified set in step 1 and does not yet touch catalog drafting. Its leave-one-out design gives it a machine-checkable ground truth (the removed curated record) instead of a human label, which covers recall and validator/reviewer false-reject rate without a grader session; `--grade` against `grades.template.json` still supplies human-judged precision when wanted. The September 9 validator-recall run is the first repeated, numbers-on-record use of it for exactly the purpose step 5 describes ("rerun affected cases and an unchanged holdout; retain prior results"): a same-day before/after pair, brief instructions, seed 1, 24 documents — recall 0.50→0.55, precision 0.92→0.77, validator false-rejects 5→2 — is retained in the Operating Guide rather than only in a throwaway terminal run. Treat this as calibration having begun on the extraction/screening lane, not as satisfying the full ten-purposive-plus-ten-sampled human-reviewed suite; the precision drop recorded alongside the recall gain is exactly the kind of trade-off this section asks a human to weigh before a longer production run, not evidence to be pattern-matched away.
