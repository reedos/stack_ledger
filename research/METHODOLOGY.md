...
For 2028–2030, an amber companion series uses AEO2026 Table 8, row 55, Counterfactual Baseline, released April 8, 2026 (NEMS cb2026.d021826b). These broader total-net-generation values include small on-site systems and are stored under a separate metric. They are not utility-scale projections and must never be appended to that metric by the daily runner. A scope marker, chart-face warning, distinct hatch color and table labels expose the boundary. Do not infer a comparable growth rate from the 2027 STEO value to the 2028 AEO value. This is a comparison of two published outlooks, not a blended forecast or current-policy promise. XLSX and PDF extraction in this addition was curated; it does not add those formats to unattended extraction.
