...
An unmapped tailnet login refuses every route for that request — status, findings, GPU telemetry, everything — with a clear 403, never a silent read-only downgrade to some other identity's data. `GET <mount>/` (or `/`, if Serve has already stripped the mount) with a resolved tailnet identity 302-redirects to `<mount>/<token>/`, so the owner's phone bookmark (the stable root URL, not the per-launch token) keeps working across restarts even though the token changes every launch. Mutations still require the served origin (`https://<hostname>` on tailnet, `http://127.0.0.1:<port>` on loopback) and the session-key header, exactly as before. Every editorial event recorded through the panel now carries `channel` and, on tailnet, `login`, so the audit trail shows where a decision was made — a CLI-recorded decision (no identity available) keeps its original shape.
...
Three tabs replace the old two: **Decisions** (default), **Session**, **Activity** — top tabs on desktop, a sticky bottom bar on phones under 720px. **Decisions** merges pending catalog packages, pending discovery findings and pending visual recommendations into one ranked "Needs your decision" feed, each card keeping its full per-type controls (the same Investigate/Defer/Reject or Approve/Reject/Apply-and-publish forms as before — this is not a summary view). A catalog package's evidence now shows its registered source rank and an "auto-apply eligible: yes/no + reason" line (`publication_policy.source_ranks`/`eligible`), and a package that was pushed but not yet confirmed deployed shows "Pushed · awaiting deployment check" instead of hiding inside the generic approved bucket. Applied/rejected/deferred items, and the full filterable findings/catalog-package/visual-recommendation browsers that existed before, sit behind a collapsed "History & everything else" fold below the feed. **Session** is the prior manual-controls tab unchanged, except the live batch output starts inside a closed `<details>` on phones (open on desktop) and the GPU utilization/temperature charts start hidden behind a "Show GPU charts" toggle on phones. **Activity** is a read-only log of what the system did on its own — `publication-policy`-authored catalog approvals/applications from the editorial-events log, and `.local/digest/*.json` importer digests when present (nothing renders if there are none) — kept separate from decisions a person made.
