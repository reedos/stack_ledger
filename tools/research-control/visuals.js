...
(()=>{
  ...
  function buildVisualCard(p){
      ...
      form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;try{await post('visual-review',{id:p.id,decision:decision.value,rationale:reason.value,confirmed:confirm.checked,proposal_hash:p.proposal_hash,review_hash:p.review_hash,reconsider_after:decision.value==='deferred'?date.value+'T23:59:59Z':null});await load();status.textContent='Decision saved. Nothing applied or published.';}catch(e){message.textContent=e.message;submit.disabled=!owner;}};
      return card;
  }
  function draw(){
    ...
    for(const p of selected.slice(0,limit))list.append(buildVisualCard(p));
  ...
  async function load(){status.textContent='Loading private recommendations…';try{const r=await fetch('visuals');if(r.status===404)throw Error('Reopen Research-Control.cmd to load the visual recommendations server.');const data=await r.json();if(!r.ok)throw Error(data.error||'Could not load recommendations');rows=data.proposals;owner=data.reviewer;assess.disabled=!owner;digest.replaceChildren(node('summary','Assessment: KEEP, evidence gaps and unassessed displays'));
    ...
    window.__pending.visuals=rows.filter(p=>p.status==='pending_review').map(p=>({rank:10,created_at:p.created_at,el:buildVisualCard(p)}));
  ...
  el('decisions-tab').addEventListener('click',load);
  load();
})();
