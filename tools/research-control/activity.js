...
(()=>{
  ...
  function reportCard(e,owner){
    ...
    if(!e.retracted)card.append(retractForm(e,owner));
    return card;
  }
  async function load(){
    ...
    try{
      ...
      reports.append(node('p',data.reviewer?`Reviewing as ${data.reviewer}. Additions publish automatically; Retract is the one human action.`:'Read-only: this account is not an authorized reviewer.','hint'));
      if(!data.reports.length)reports.append(node('p','No grade C/D reports published yet.','empty-inbox'));
      for(const e of data.reports)reports.append(reportCard(e,data.reviewer));
      box.append(reports);
