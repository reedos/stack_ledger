...
function companyCard(c) {
 ...
 return `<article class="company-card" id="${esc(c.id)}" style="--accent:${accent}">
  ...
  <div class="company-revenue ${c.revenue_kind==='run-rate'?'run-rate':''}"><span class="eyebrow">${c.revenue_kind==='run-rate'?'ANNUALIZED RUN RATE':'REPORTED ANNUAL REVENUE'}</span><strong>${esc(valueOf(o))}<small>${esc(m.unit)}</small></strong><p>${esc(o.period)}</p></div>
...
function companiesPage() {
 ...
 <div class="directory-tools"><label for="company-search" class="sr-only">Search companies or capabilities</label><input type="search" class="search" id="company-search" placeholder="Search companies or capabilities: memory, turbines, Claude…"><label for="revenue-basis">Revenue basis <select class="select-control" id="revenue-basis"><option value="all">All disclosures</option><option value="annual">Annual revenue only</option><option value="run-rate">Run rates only</option></select></label></div>
...
function industryPage() {
 ...
 root.innerHTML=`<section class="page-hero ecosystem-hero"><div class="eyebrow">JOBS, FACTORIES & THE LONG VIEW</div><h1>Intelligence has<br>a physical footprint.</h1><p>The AI buildout reaches into factories, power equipment and local communities. Follow the evidence from a company’s plans to operating capacity and lasting work.</p>${researchStamp()}</section>
