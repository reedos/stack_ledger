...
def outlook_attribution(outlook,sources):
    """Which outlook bars rest on news reporting rather than the company's own channel.

    Derived from each source's provenance, so the sentence changes when a source does. It was
    once typed by hand and went on naming a company after its figure had moved to a primary.
    """
    news=[(row['name'],sources[o['source']]['publisher']) for row,o,forecast in outlook if forecast and sources[o['source']].get('provenance')=='news']
    if not news:return 'Every outlook comes from the company’s own channel.'
    by={}
    for name,publisher in news:by.setdefault(publisher,[]).append(name)
    parts=[]
    for publisher,names in by.items():
        who=names[0] if len(names)==1 else ', '.join(names[:-1])+' and '+names[-1]
        parts.append(f'{who}{"’s outlook is" if len(names)==1 else " outlooks are"} attributed to {publisher} reporting of the earnings call')
    return '; '.join(parts)+'; the other outlooks come from the companies’ own channels.'
def capital(ledger,x,base):
    ...
    attribution=outlook_attribution(outlook,sources)
