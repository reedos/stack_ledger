...
def capital(ledger,x,base):
    ...
    years=[o['year'] for _,rs in series for o in rs]+[year for year,_,_ in forecast_totals]+[year for year,_ in archive_totals];lo,hi=min(years),max(years)
    totals=capital_totals(series)
    top=max(50,((int(max([o['value'] for _,rs in series for o in rs]+[value for _,value in totals]+[high for _,_,high in forecast_totals]+[value for _,value in archive_totals]))//50)+1)*50)
    ...
    if archive_totals:
        path=''
        for i,(year,value) in enumerate(archive_totals):
            command='L' if i and year==archive_totals[i-1][0]+1 else 'M'
            path+=f'{command}{xp(year):.2f} {yp(value):.2f}'
            svg+=f'<circle class="capital-archive" cx="{xp(year):.2f}" cy="{yp(value):.2f}" r="4" fill="#b0a0da"><title>Older September 2025 forecast · CY{year}: {value:g} USD billion · all five companies</title></circle>'
        ...
        svg+=f'<path class="capital-archive" d="{path}" stroke="#b0a0da" stroke-width="2.5" stroke-dasharray="2 5" fill="none"/>'
        legend+='<span style="--series:#b0a0da"><i></i>Older forecast · September 2025<small>All five, CY2026–2029; not updated guidance</small></span>'
...
def capability_svg(c,compact=False):
    ...
    for r in c['rows']:
        ...
        svg+=f'<circle style="{"" if r in rs else "display:none"}" class="eci-point" data-model="{r["id"]}" cx="{xp(r["released"]):.2f}" cy="{yp(r["score"]):.2f}" r="{2.5 if compact else 3.8}" fill="{color}" opacity=".7"><title>{e(r["name"])} · {r["released"]} · ECI {r["score"]:g}{interval}</title></circle>'
    return svg+'</svg>'
