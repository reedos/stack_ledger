    ...
    if True:
        ...
        with research.lock():
            ...
            try:
                ...
                if matches:
                    append_event(root,dict(dict(id=rid,kind='catalog_change',status='applied',reviewer=reviewer,at=now(),proposal_hash=digest(p),commit=receipt['commit']),**channel_fields(identity)))
