...
def main():
    ...
    payload=[sys.executable,str(ROOT/'scripts/nightly.py')]
    name=NAME
    declaration='stack-ledger-daily-v2'
    description='Nightly orchestrator: stale-lock recovery, dataset imports, the 2-7 AM Pacific research session, automatic publication of admitted catalog packages, a health/stale-figure report, preview pruning and a digest. No chat delivery.'
    timeout='21600'  # Six-hour ceiling from 01:30, covering every stage plus graceful finalization.
    if args.editorial:
