...
# The bar is 24px type in a narrow column, so it takes the symbol; "more than 33%" wrapped onto two
# lines there and doubled the row's height. The data table underneath keeps the words, which is
# what a screen reader reads out.
PRECISION_SYMBOL = {'eq': '', 'approx': '~', 'gt': '&gt;'}


def survey_value(point):
    """The spelled-out figure, for the accessible data table.

    A survey answer the source states loosely stays loose: the New York Fed reports "just over a
    third of service firms" retrained workers and gives no exact percentage, so publishing 34%
    would invent a digit it never printed.
    """
    return f"{PRECISION_PREFIX.get(point.get('precision', 'eq'), '')}{point['value']:g}%"


def survey_bar_value(point):
    """The same figure for the bar, where space is tight."""
    return f"{PRECISION_SYMBOL.get(point.get('precision', 'eq'), '')}{point['value']:g}%"
...
def employment_feature(data, citations):
    ...
    bars=''.join(f'<div class="evidence-bar-row"><div><span>{e(p["label"])}</span><strong>{survey_bar_value(p)}</strong></div><div class="evidence-bar-track"><span class="evidence-bar" style="width:{p["value"]:g}%"></span></div></div>' for p in s['points'])
