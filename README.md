...
- `python scripts/import_qcew.py --apply` imports BLS Quarterly Census of Employment and Wages open data (public domain) for NAICS 518210 data processing and hosting and 238210 electrical contractors: quarterly private employment for every county where an accepted project has a reviewed county location, plus the national total, from 2024 Q1 onward. Suppressed quarters are omitted, never zero. Project cards in those counties show a "Local labor market" block with the latest level and the change against the same quarter a year earlier; measured county jobs sit beside the project's own promised and reported figures without being attributed to it.
