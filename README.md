...
### Reports lane (owner decision, September 9, 2026)

The model may read news articles and official social-media posts and publish what it finds, provided every item is honest about its source and how solidly official numbers support it. `source_policy.grade_for` derives a deterministic evidence grade (A/B/C/D) from a source's reviewed rank and `claim_type` — the model never assigns or sees a grade to decide with. A grade C or D item is never blended into a numeric series, a chart or a homepage headline; `validate.py` enforces this directly (an automated numeric observation must already be grade A or B). Grade C/D evidence from a registered news feed (`claim_type: news`, rank 4) or an official social account (rank 6, the same "Official social accounts" category `session_options.SOURCE_KINDS` already reserves) instead publishes automatically as a `News report`/`Social post` event: outlet, the outlet's own date, a verbatim quote, and a `confirmation` state (`unconfirmed`/`confirmed_by:<id>`/`contradicted_by:<id>`/`expired` after 90 days unconfirmed) that updates automatically, deterministically, when a later official record covers the same metric and period (`scripts/reports.py`). Registering a new host is still a reviewed source-policy change (`scripts/find_feeds.py --outlets`/`--social`); X and LinkedIn are not crawlable (X answered HTTP 402 on 2026-09-09), so their posts can only reach the ledger secondhand, through a registered news outlet's own reporting. Replacing or retracting a report is the one human action: the Research Control panel's Activity tab lists every grade C/D report with its confirmation state and a Retract control.
