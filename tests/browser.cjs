...
(async()=>{
 ...
 try{
  ...
  assert.match(await page.locator('#historical-context .scope-warning').innerText(),/High Economic Growth/);
  ...
  assert.equal(await page.locator('#application-outcomes [data-metric="tesla-robotaxi-unsupervised"] .bar').count(),1);
  assert.equal(await page.locator('#application-outcomes [data-metric="waymo-iihs-police-reportable-crash-rate"] .bar').count(),2);
  assert.equal(await page.locator('#application-outcomes [data-metric="digit-gxo-totes"] .bar').count(),1);
  assert.equal(await page.locator('#application-outcomes [data-metric="interpath001-trial-enrollment"] .bar').count(),1);
  assert.equal(await page.locator('#application-outcomes [data-metric="rec4881-polyp-reduction-week13"] .bar').count(),1);
  assert.match(await page.locator('#application-outcomes').innerText(),/trial-stage results[\s\S]*not a measured shortening/i);
