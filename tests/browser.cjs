...
(async()=>{
 ...
 try{
  ...
  await page.selectOption('#revenue-basis','annual');assert.equal(await page.locator('.company-card').count(),38);
  await page.selectOption('#revenue-basis','unavailable');assert.equal(await page.locator('.company-card').count(),5);
  assert.equal(await page.locator('.company-revenue.unavailable').count(),5);
