...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#historical-context [data-metric="us-utility-generation-history"] .bar:not(.overlay)').count(),12);
  assert.equal(await page.locator('#historical-context [data-metric="us-utility-generation-history"] .bar.overlay').count(),0);
  assert.equal(await page.locator('#historical-context [data-metric="us-utility-generation-history"] .bar:not(.overlay) rect[fill^="url"]').count(),5);
  ...
  assert.match(await page.locator('#application-outcomes').innerText(),/Tesla has not disclosed[\s\S]*paid-trips/i);
  assert.match(await page.locator('#application-outcomes').innerText(),/IIHS comparison/i);
  ...
  assert.equal(await page.locator('#outlook [data-metric="dc-electricity"] .bar:not(.overlay)').count(),4);
  assert.equal(await page.locator('#outlook [data-metric="dc-electricity"] .bar.overlay').count(),1);
  assert.match(await page.locator('#outlook').innerText(),/Lift-Off is the taller bar/i);
