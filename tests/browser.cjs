...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#historical-context [data-metric="us-utility-generation-history"] .line-point[data-series="us-utility-generation-history"]').count(),9);
  assert.equal(await page.locator('#historical-context [data-metric="us-utility-generation-history"] .line-point[data-series="us-total-generation-aeo2026"]').count(),8);
  assert.equal(await page.locator('#historical-context [data-metric="us-utility-generation-history"] .line-point[data-series="us-total-generation-aeo2026-high-growth"]').count(),8);
  assert.equal(await page.locator('#historical-context .scope-break-marker').count(),1);
  assert.match(await page.locator('#historical-context .scope-warning').textContent(),/High Economic Growth/);
  ...
  assert.match(await page.locator('#building-workforce').innerText(),/72,700[\s\S]*replacement needs/);
  ...
  assert.equal(await page.locator('#metric-chart .line-point').count(),25);
  await page.locator('#metric-chart .history-window').check();assert.equal(await page.locator('#metric-chart .line-point').count(),30);
  await page.setViewportSize({width:390,height:844});
  assert.equal(await page.locator('#metric-chart .line-point').count(),30);
