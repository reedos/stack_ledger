...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#eci-model option').count(),defaultCount);
  ...
  assert.equal(await page.locator('#eci-model option').count(),all.filter(r=>r.country==='France').length);
  await page.selectOption('#eci-color','access');await page.locator('#eci-show-all').click();
  assert.equal(await page.locator('#eci-model option').count(),all.length);
  await page.locator('#eci-search').fill('no-such-model');assert.equal(await page.locator('#eci-model option').count(),0);assert.match(await page.locator('#eci-selection').innerText(),/No models match/);
  await page.locator('#eci-reset').click();assert.equal(await page.locator('#eci-model option').count(),defaultCount);
