...
(async()=>{
 ...
 try{
  ...
  assert.match(await page.locator('#map-count').innerText(),new RegExp(`${delivery.projects.filter(p=>p.map_location).length} mapped`));
  ...
  await page.locator('#map-us').click();await page.locator('.map-marker').first().click();assert.match(await page.locator('#map-selection').innerText(),/Approximate county/);
  ...
  assert.equal(await page.locator('#project-results .delivery-card').count(),delivery.projects.filter(p=>!p.map_location).length);
  ...
  assert.match(await page.locator('#map-count').innerText(),new RegExp(`${delivery.projects.filter(p=>p.layer==='chips'&&p.map_location).length} mapped`));
  ...
  assert.equal(await page.locator('#eci-model option').count(),expansion.capabilities.rows.length);
  await page.selectOption('#eci-developer','OpenAI');await page.selectOption('#eci-access','Closed weights');
  const selected=expansion.capabilities.rows.filter(r=>r.organization==='OpenAI'&&r.access==='Closed weights');
  assert.equal(await page.locator('#eci-model option').count(),selected.length);
  assert.equal(await page.locator('#eci-table tr').count(),selected.length);
  await page.locator('#eci-frontier-only').check();assert.ok(await page.locator('#eci-model option').count()<selected.length);
  await page.selectOption('#eci-developer','Anthropic');await page.selectOption('#eci-access','Open weights');
  assert.equal(await page.locator('#eci-model option').count(),0);assert.match(await page.locator('#eci-selection').innerText(),/No models match/);
  await page.selectOption('#eci-access','all');await page.selectOption('#eci-developer','all');await page.locator('#eci-frontier-only').uncheck();
