...
class HomepageContracts(unittest.TestCase):
    ...
    def test_offline_end_to_end_fixture_review_apply_build(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            review_at=max(fx.AS_OF,config['reviewed_at'])
            snap=review.assemble(root,review_at,'offline-fixture-mock')
            ...
            review.record_review(root,p['recommendation_id'],'approved','reedos','Fixture human review of accepted CoWoS ranges.',review_at,human_confirm=True)
